...
static int
add_timeline_fence_array(struct i915_ttm_execbuffer *eb,
...
static int add_fence_array(struct i915_ttm_execbuffer *eb)
{
...
static void signal_fence_array(const struct i915_ttm_execbuffer *eb)
{
...
{
	struct i915_ttm_execbuffer *eb = data;
		       ...
		       struct drm_i915_gem_exec_object2 *exec)
{
	...
	r = parse_execbuf2_extensions(args, &eb);
	if (r)
		goto err_ext;

	r = add_fence_array(&eb);
	if (r)
		goto err_ext;
...
#undef IN_FENCES
	...
	i915_ttm_bo_list_for_each_entry(e, eb.bo_list) {
		...
		if (!atomic_fetch_inc(&e->vma->open_count))
			i915_vma_reopen(e->vma);

		if (drm_mm_node_allocated(&e->vma->node) &&
		    (e->vma->node.start != (e->pin_flags & PIN_OFFSET_MASK))) {
			r = i915_vma_unbind(e->vma);
			if (r)
				return r;
		}
